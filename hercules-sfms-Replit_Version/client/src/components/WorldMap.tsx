...
import { getFacilityLocation, getFacilityStatus, getFacilityEfficiency, getFacilityDailyProduction, getFacilityLatitude, getFacilityLongitude } from "@/lib/facility";
...
export function WorldMap({ facilities, onFacilityClick }: WorldMapProps) {
  ...
  const getFacilityPosition = (facility: Facility) => {
    ...
    const latPercent = ((latitude - indiaLatRange[0]) / (indiaLatRange[1] - indiaLatRange[0])) * 100;
    const lngPercent = ((longitude - indiaLngRange[0]) / (indiaLngRange[1] - indiaLngRange[0])) * 100;
    ...
    <div className="col-span-5 row-span-3 holographic rounded-lg p-4 relative overflow-hidden">
      ...
      <div className="absolute inset-4 w-full h-5/6">
        {facilities.map((facility) => {
          ...
          const statusColor = getStatusColor(getFacilityStatus(facility));
            ...
            >
              ...
              {hoveredFacility?.id === facility.id && (
                <div className="absolute bottom-6 left-1/2 transform -translate-x-1/2 bg-[hsl(240,25%,6%)] border border-[hsl(180,100%,50%)] rounded px-2 py-1 text-xs whitespace-nowrap z-20 pointer-events-none">
                  <div className="text-[hsl(180,100%,50%)] font-mono">{getFacilityLocation(facility).toUpperCase()}</div>
                  <div className="text-white">{getFacilityDailyProduction(facility).toLocaleString()} m³/day</div>
                  <div style={{ color: statusColor }}>{getFacilityEfficiency(facility)}% efficiency</div>
                </div>
              )}
            </div>
          );
