...
import { getFacilityLatitude, getFacilityLongitude, getFacilityLocation, getFacilityStatus, getFacilityEfficiency, getFacilityDailyProduction } from "@/lib/facility";
...
export function HolographicMap({ facilities, onFacilityClick }: HolographicMapProps) {
  ...
  const getFacilityPosition = (facility: Facility) => {
    ...
    const latitude = getFacilityLatitude(facility);
    const longitude = getFacilityLongitude(facility);
    const latNorm = (latitude - saudiLatRange[0]) / (saudiLatRange[1] - saudiLatRange[0]);
    const lngNorm = (longitude - saudiLngRange[0]) / (saudiLngRange[1] - saudiLngRange[0]);
      ...
      <div className="absolute inset-4 w-full h-5/6">
        ...
        >
          ...
          {facilities.map((facility, index) => {
            ...
            return (
              <g key={facility.id}>
                ...
                <circle 
                  cx={x} cy={y} r="4" 
                  fill={getStatusColor(getFacilityStatus(facility))} 
          ...
          <g stroke="hsl(180, 100%, 60%)" strokeWidth="1.5" opacity="0.8">
            {(() => {
              const riyadh = facilities.find(f => getFacilityLocation(f) === "Riyadh");
      ...
      {hoveredFacility && (
        <div className="absolute z-50 pointer-events-none">
          ...
          >
            ...
            <div className="text-cyan-400">{getFacilityLocation(hoveredFacility)}</div>
            <div className="flex justify-between gap-4 mt-1">
              <span>Status: <span className="text-green-400">{getFacilityStatus(hoveredFacility)}</span></span>
              <span>Efficiency: <span className="text-blue-400">{getFacilityEfficiency(hoveredFacility)}%</span></span>
            </div>
            <div className="text-gray-400">Production: {getFacilityDailyProduction(hoveredFacility).toLocaleString()} m³/day</div>
          </div>
        </div>
      )}
    </div>
