...
import { getFacilityLocation, getFacilityStatus, getFacilityEfficiency, getFacilityDailyProduction } from "@/lib/facility";
...
export function FacilityCard({ facility, onClick }: FacilityCardProps) {
  ...
  const efficiency = getFacilityEfficiency(facility);
  const dailyProduction = getFacilityDailyProduction(facility);
  const locationLabel = getFacilityLocation(facility)
    .replace(' Water Treatment Plant', '')
    .toUpperCase();
  const statusColor = getStatusColor(status);
  const { circumference, offset } = calculateCircularProgress(efficiency);
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex items-center justify-between mt-1">
          <div className="text-xs font-mono text-white">
            {(dailyProduction / 1000).toFixed(1)}K m³
          </div>
          <div 
